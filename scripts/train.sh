...
OMP_NUM_THREADS=1 srun -p ${machines} --gres=gpu:${num_gpus} -n${num_cpus} --cpu_bind=threads --ntasks-per-node=${num_cpus} --cpus-per-task=1 --job-name=${jobname} --mpi=pmi2 --kill-on-bad-exit=1 \
